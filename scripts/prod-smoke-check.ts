...
if (!apiAccessKey) {
  // #252's shared-secret gate has not reached `main` yet (#276) — as of
  // 2026-09-03 there is no secret to present, so this is expected and not
  // a coverage gap today. Left as a WARN rather than removed because the
  // moment #252 does release to main, a 401 needs to stop being silently
  // misread as "dev tooling exposed" (see #262) — this is the log line
  // that will remind whoever wires PROD_API_ACCESS_KEY up then.
  console.log(
    ...
    "Not a coverage gap today: #252 hasn't reached main yet (#276), so there's no key to provide."
  );
}
